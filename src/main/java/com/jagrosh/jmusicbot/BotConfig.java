  ...
  public void load() {
    ...
    try {
      ...
      if (token == null || token.isEmpty() || token.equalsIgnoreCase("BOT_TOKEN_HERE")) {
        throw new ConfigException.Generic("Provided token is invalid!");
      }

      // validate bot owner
      if (owner <= 0) {
        throw new ConfigException.Generic("Provided owner is invalid!");
      }

      // if we get through the whole config, it's good to go
      valid = true;
    } catch (ConfigException ex) {
      log.error("Config could not be loaded!", ex);
    }
  }
